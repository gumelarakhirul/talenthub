...
import { scrapeInstagramProfiles, scrapeTiktokProfiles, RawPost, RawProfile } from './apify';
...
// Dipisah dari processCreator supaya bisa dipakai juga oleh quick-search
// (search 1 akun manual), yang gak selalu punya categoryId dari seed entry.
// Melakukan: hitung metrics, deteksi endorse, upsert creator + posts,
// dan cari username baru dari bio/mention.
export async function persistCreatorProfile(
  profile: RawProfile,
  ...
  const creator = await prisma.mst_creators.upsert({
    ...
    update: {
      ...
      ...(options.cityId != null ? { city_id: options.cityId } : {}),
  ...
  console.log(`  [OK] persistCreatorProfile: ${profile.username}, id ${creator.id}, ${savedPosts}/${profile.posts.length} post`);
...
export async function processCreator(entry: SeedEntry) {
  ...
  // 4-7. Hitung metrics, simpan creator + posts, cari username baru
  const creatorId = await persistCreatorProfile(profile, { categoryId: category.id, cityId });
  console.log(`  [OK] creator id ${creatorId}, tier ${calculateTier(profile.followers)}`);

  return { status: 'success', username: entry.username, creatorId };
}
